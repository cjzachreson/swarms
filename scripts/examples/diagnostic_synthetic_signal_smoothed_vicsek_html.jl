...
function run_example()
     ...
     smoothing = ExponentialSmoothingConfig(0.05)
     ...
     smoothed_features = smooth_feature_frames(analysis.features, smoothing)
     run_frames = run_controlled_simulation(state, base_params, smoothed_features, mapping, 1.0, rng)
     swarm_frames = [frame.swarm for frame in run_frames]
     parameter_frames = [frame.params for frame in run_frames]

     output_path = joinpath("outputs", "diagnostic_synthetic_signal_smoothed_vicsek.html")
