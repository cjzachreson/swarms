function synthetic_feature_frame(time::Real)
     ...
     t = Float64(time)
     rms = normalized_wave(t, 0.55)
     low_band = normalized_wave(t, 0.18)
     mid_band = normalized_wave(t + 0.6, 0.37)
     high_band = normalized_wave(t + 1.2, 0.83)
     spectral_centroid = clamp(0.25 + 0.55 * high_band + 0.20 * mid_band, 0.0, 1.0)
     onset_strength = pulse_train(t, 1.0, 0.10)
